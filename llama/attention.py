...
class AttentionLayer(nn.Module):
    def __init__(self, token_dimensions: int, number_of_heads: int ):
        ...
        self.head_dimensions = token_dimensions // number_of_heads
        self.scale = self.head_dimensions** -0.5
        ...
        self.positional_rope = nn.RoPE(self.head_dimensions, traditional=True)

        self.query_weights = nn.Linear(token_dimensions, self.number_of_heads * self.head_dimensions, bias=False)
        self.key_weights = nn.Linear(token_dimensions, self.number_of_heads * self.head_dimensions, bias=False)
        self.value_weights = nn.Linear(token_dimensions, self.number_of_heads * self.head_dimensions, bias=False)

        self.output_weights = nn.Linear(self.number_of_heads * self.head_dimensions, token_dimensions, bias=False)
    ...
    def __call__(self, x, mask=None, key_value_cache=None):
        ...
        batch_size, sequence_length, token_dimensions = projected_queries.shape
        ...
        if key_value_cache is not None:
            ...
            projected_queries = self.rope(
                projected_queries,
                offset=projected_key_cache.shape[2]
            )
            projected_keys = self.rope(
                projected_keys,
                offset=projected_key_cache.shape[2]
            )
            ...
            projected_queries = self.rope(projected_queries)
            projected_keys = self.rope(projected_keys)
        ...
        attention_output = (attention @ projected_values).transpose(0, 2, 1, 3).reshape(
            batch_size,
            sequence_length,
            self.number_of_heads * self.head_dimensions
        )
        return self.output_weights(attention_output), (projected_keys, projected_values)
